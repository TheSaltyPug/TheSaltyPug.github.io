    ...
    async function getRepos() {
      ...
      let repos = result.items // Your repo data
        repos.sort((a, b) => { return a.create_at - b.create_at; });   
        repos.reverse();
      ...
      for(p = 0; p < 3; p++){
          ...
          project_image.src = "https://raw.githubusercontent.com/" + repos[p].full_name + "/master/index.png"
          project_image.alt = repos[p].name + " image"

          const project_name = document.createElement("p")
          project_name.className = "header"
          project_name.innerText = repos[p].name

          const project_desc = document.createElement("p")
          project_desc.className = "body"
          project_desc.innerText = repos[p].description
